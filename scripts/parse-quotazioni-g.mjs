...
const giornata = process.argv[2] || "3";
const xlsArg = process.argv[3];
const xlsPath = path.join(root, xlsArg || `fc_quotazioni_mondiale_${giornata.padStart(2, "0")}.xls`);
const listonePath = path.join(root, "listone.json");
const outPath = path.join(root, `listone_${giornata}g.json`);
...
if (!fs.existsSync(xlsPath)) {
  console.error("File non trovato:", xlsPath);
  process.exit(1);
}
...
console.log("Excel:", path.basename(xlsPath), "·", wb.SheetNames[0], "·", rows.length, "righe");
...
console.log("Creato", path.basename(outPath));
console.log("Giocatori:", count(out));
console.log("Quotazioni aggiornate vs listone.json:", updated);
console.log("Senza match in excel (valore iniziale):", missing.length);
if (missing.length) {
  console.log(missing.map((m) => `${m.nome} (${m.nazione}) ${m.valore}`).join("\n"));
}
